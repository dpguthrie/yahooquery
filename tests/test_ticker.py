...
TICKERS = [
    Ticker('aapl'), Ticker('aapl ^GSPC btcusd=x brk-b logo.is'),
    Ticker(['aapl', 'aaapl']), Ticker('hasgx'), Ticker('aapl', formatted=False)
]
...
def test_all_endpoints(ticker):
    ...
    assert sorted(list(data.keys())) == sorted(ticker.symbols)
